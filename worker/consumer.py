...
def consume(host):
    """
    Consumer หลักที่รับ job จาก RabbitMQ
    """
    ...
    try:
        ...
        # ประกาศ queue (ถ้ายังไม่มี)
        ch.queue_declare(queue="router_jobs", durable=True)
        
        # ตั้งค่า prefetch เพื่อไม่ให้ worker รับงานมากเกินไป
        ...
        # เริ่มรับ message (ไม่ใช้ auto_ack เพื่อความปลอดภัย)
        ch.basic_consume(
            queue="router_jobs",
            on_message_callback=callback,
            auto_ack=False  # เปลี่ยนเป็น False เพื่อ manual ack
        )
